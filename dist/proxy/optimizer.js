...
function optimizeQuery(sql, tableStats = {}, knownColumns = DEFAULT_COLUMNS) {
    const result = {
        originalQuery: sql,
        optimizedQuery: sql,
    ...
    try {
        ...
        if (selectStarMatch) {
            ...
            if (cols && cols.length > 0) {
                ...
                result.inversionHints.push(`Use SELECT ${cols.join(', ')} FROM ${tableName} instead of SELECT *`);
            }
        }
        // Rule 2: Auto LIMIT
        ...
        if (!hasLimit && !hasAggregate) {
            ...
            result.inversionHints.push(`Add LIMIT to your query — table '${tableName}' can be large`);
            if (rowCount > 1000) {
