...
function printLineage() {
    ...
    const records = (0, lineage_1.readLineage)(last);
    if (records.length === 0) {
...
function printChains() {
    const dir = (0, path_1.join)((0, os_1.homedir)(), '.flowkernel', 'lineage');
    const date = new Date().toISOString().slice(0, 10);
    const filePath = (0, path_1.join)(dir, `${date}.jsonl`);
    if (!(0, fs_1.existsSync)(filePath)) {
        console.log('No chain data found. Run the proxy first.');
        return;
    }
    const lines = (0, fs_1.readFileSync)(filePath, 'utf8')
        .split('\n')
        .filter(Boolean)
        .map(l => JSON.parse(l));
    // Group by session
    ...
    for (const [sessionId, records] of Object.entries(sessions)) {
        const tables = [...new Set(records.map((r) => {
...
function printStatus() {
    ...
    const totalRowsSaved = lines.reduce((sum, r) => {
        return sum + (r.estimatedRowsSaved ?? 0);
    }, 0);
    const uniqueSessions = new Set(lines.map((r) => r.sessionId)).size;
    const appliedCounts = {};
    for (const r of lines) {
